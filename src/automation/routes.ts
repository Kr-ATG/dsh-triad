 ...
 *   GET  /api/triad-automation/cron                 → { jobs }
 *   POST /api/triad-automation/cron                 → { action, ...params }
 *         add / remove / toggle / update / apply_suggestion
 *   GET  /api/triad-automation/runs?jobId=&limit=   → { runs }（运行历史）
 *   GET  /api/triad-automation/suggestions          → { suggestions }（待确认建议）
 *   POST /api/triad-automation/suggestions          → { action: dismiss|apply }
 *   GET  /api/triad-automation/events?since=<seq>   → { events }（完成事件，供 toast）
 *   GET  /api/triad-automation/settings             → { autoApprove }
 *   POST /api/triad-automation/settings             → { autoApprove }（写入自动化目录 settings.json）
 *
...
export const ROUTE_PREFIX = '/api/triad-automation'
...
export interface RouteDeps {
  ...
  /** 设置读写面（自动化目录 settings.json 持久化的 autoApprove）。 */
  settings: {
