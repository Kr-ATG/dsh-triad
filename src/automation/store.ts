...
/** 旧数据根（dsh-webui 时代）：仅用于首次启动自动迁移。 */
export function automationLegacyRoot(): string {
  const dshHome = process.env.DSH_HOME ?? join(homedir(), '.dsh')
  return join(dshHome, 'automation', 'dsh-webui')
}

/** 数据根：${DSH_HOME:-~/.dsh}/automation/dsh-triad/。 */
export function automationDataRoot(): string {
  const dshHome = process.env.DSH_HOME ?? join(homedir(), '.dsh')
  return join(dshHome, 'automation', 'dsh-triad')
}

/** 首次启动迁移旧数据：新路径无任务文件但旧路径有时整目录拷贝（只做一次）。 */
export function migrateLegacyAutomationData(): void {
  try {
    const next = automationDataRoot()
    const legacy = automationLegacyRoot()
    if (next === legacy) return
    if (existsSync(join(next, 'cron-jobs.json'))) return
    if (!existsSync(join(legacy, 'cron-jobs.json'))) return
